...
object LevelBuildersRemoveCommand : CommandExecutor {
    override fun run(sender: CommandSender, args: Array<out Any>) {
        ...
        if (!level.builders.contains(player.uniqueId)) {
            sender.sendMessage("${ChatColor.RED}This player is not a builder!")
            return
        }

        level.builders.remove(player.uniqueId)
        sender.sendMessage("${ChatColor.GOLD}${player.name} ${ChatColor.GREEN}has been removed from the builders list.")
    }
}